...
Per ricavare l'equazione del moto delle due masse, assumiamo che le dimensioni de sistema siano trascurabili rispetto alla scala di variazione del campo gravitazionale: assumiamo perciò $L \ll \lambdabar$.
...
Inoltre, facendo riferimento alla sezione 1.3.3 del Maggiore, ci poniamo nel proper detector frame: così facendo possiamo utilizzare il fatto che l'equazione della deviazione geodetica si riduce a quella di una forza newtoniana (equazione 1.96).
...
dove $r$ è la distanza dalla sorgente. L'ampiezza $h_+$ è calcolata a partire dall'equazione 3.72, considerando che, nel nostro caso, è conveniente definire l'angolo polare $\theta$ del sistema di coordinate sferiche a partire dall'asse $x$ ($\theta=0$), mentre l'angolo azimutale $\varphi$ descrive le rotazioni nel piano $z-y$, sotto le quali la potenza irradiata è invariante.
...
\begin{equation} \label{eq:integraleperiodo}
    \frac{\omega}{2 \pi} \left[ \int_0^{\pi/\omega} \sin^2(2\omega t) dt + \int_{\pi/\omega}^{2\pi/\omega} \sin^2(2\omega t) dt \right] = \frac{\omega}{2 \pi} \left[ \frac{\pi}{2 \omega} + \frac{\pi}{2 \omega} \right] = \frac{1}{2} \ .
\end{equation}
...
Inoltre, quando la massa $m_1$ viene tirata in basso, la massa $m_2$ sente una forza in verso opposto.
...
\begin{itemize}
    ...
    \item Nella derivazione dell'equazione del moto, abbiamo assunto un segnale periodico, perciò abbiamo supposto che l'onda incida sul sistema per un tempo molto maggiore del tempo scala di dissipazione delle oscillazioni dovuto all'attrito.
        In altre parole, l'equazione del moto \eqref{eq:eqmoto} è valida solo se il segnale dura un tempo $\Delta t \gg \tau$, con $\tau$ dato dall'equazione \eqref{eq:tau}, che vale $\tau \simeq 0.32 \times 10^3$ s.
...
È lo sviluppo di Taylor della distribuzione di temperatura $T$ al variare dell'angolo $\B\theta$ bidimensionale al secondo ordine. Con $T$ si intende la distribuzione di temperatura al momento di emissione della CMB. \hfill $\blacksquare$
...
\begin{itemize}
    ...
    \item $\displaystyle \delta_\Theta^{(2)} (\B l) = \frac{1}{2} \int \frac{d^2 \B l_1}{(2\pi)^2} \int \frac{d^2 \B l_2}{(2\pi)^2} [\B l_1 \cdot (\B l - \B l_1 - \B l_2)] [\B l_2 \cdot (\B l - \B l_1 - \B l_2)] \phi_L(\B l_1) \phi_L(\B l_2) \Theta(\B l - \B l_1 - \B l_2)$
...
\begin{equation}
    C_{\text{obs}}(l) = \langle \delta_\Theta^{(1)}(\B l) \delta_\Theta^{(1)}(\B l) \rangle + \langle \delta_\Theta^{(1)}(\B l) \delta_\Theta^{(2)}(\B l) \rangle + \langle \delta_\Theta^{(2)}(\B l) \delta_\Theta^{(1)}(\B l) \rangle + \langle \delta_\Theta^{(2)}(\B l) \delta_\Theta^{(2)}(\B l) \rangle \ ,
\end{equation}
...
\begin{equation}
\begin{aligned}
    \langle \delta&_\Theta^{(1)}(\B l) \delta_\Theta^{(2)}(\B l) \rangle + \langle \delta_\Theta^{(2)}(\B l) \delta_\Theta^{(1)}(\B l) \rangle = \\
...
In tal modo si ha che $\B L = \B l$, compare un segno ´$-$' davanti all'integrale ed in aggiunta i due contributi si sommano, dando:

\begin{equation} \label{eq:quasiC13}
    \langle \delta_\Theta^{(1)}(\B l) \delta_\Theta^{(2)}(\B l) \rangle + \langle \delta_\Theta^{(2)}(\B l) \delta_\Theta^{(1)}(\B l) \rangle = - \int \frac{d^2 \B l_1}{(2\pi)^2} (\boldsymbol{l} \cdot \boldsymbol{l}_1)^2 C_{\phi\phi}(l_1) C(l)
\end{equation}
...
\begin{equation}
    \langle \delta_\Theta^{(1)}(\B l) \delta_\Theta^{(2)}(\B l) \rangle + \langle \delta_\Theta^{(2)}(\B l) \delta_\Theta^{(1)}(\B l) \rangle = -\frac{l^2}{2} \left[ \int \frac{d^2 \B l_1}{(2\pi)^2} l_1^2 C_{\phi_{\text{L}}\phi_{\text{L}}}(l_1) \right] C(l) \equiv 2C^{(13)}(l) \ .
\end{equation}
...
Infine, integrando i due termini, entrambe le delta di Dirac impongono $\B l_1 = \B l - \B l'$, e si ottiene:
...
I fotoni provenienti dalla CMB sono stati emessi a redshift $z^* = z_{\text{ricomb}} \simeq 1089$, e prima di arrivare a noi hanno subito numerose deflessioni dovute alle fluttuazioni del potenziale gravitazionale dovute alla struttura su larga scala.
...
Per calcolare $C(l)$, che in questa sezione indicheremo con $C_{TT}$, occorre innanzitutto avere informazioni su $C_{\phi_L \phi_L}(l)$ (d'ora in poi indicato con $C{\phi\phi}$): dall'equazione 13.44 del Dodelson, possiamo dire che:
...
dove $\chi^* = \chi(z^*)$.
Per calcolare $C_{\kappa\kappa}$, dunque, dobbiamo far evolvere lo spettro di potenza della materia dato, i.e. $P(k,z=0)$, fino a $z^*$, per farlo è necessario definire il fattore di crescita $D(z)$, dato dall'equazione 8.76 del Dodelson:
...
Gravitational lensing of the Cosmic Microwave Background (CMB) provides a powerful probe of the projected mass distribution in the universe. As CMB photons propagate from the last scattering surface ($z_{\text{recomb}} \approx 1089$) to the observer, their trajectories are deflected by the gradients of the gravitational potential generated by large scale structure. In this exercise, we evaluate the impact of this remapping on the CMB power spectra, assuming a standard flat $\Lambda$CDM cosmology with $\Omega_m = 0.3$ and $H_0 = 68$ km/s/Mpc. 
